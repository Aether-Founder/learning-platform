...
function textValue(value: unknown): string {
  if (value && typeof value === 'object' && 'start' in value)
    return String((value as { start: unknown }).start);
...
export default function CalendarView({ databaseId, datePropertyId }: CalendarViewProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: propertyData }, { data: entryData }] = await Promise.all([
        ...
        client
          .from('notion_entries')
          .select('id, notion_property_values(property_id, value)')
          .eq('database_id', databaseId)
          .order('sort_order'),
      ]);
      if (cancelled) return;
  ...
  const dateProperty =
    properties.find((property) => property.id === datePropertyId) ||
  ...
  const entriesByDay = useMemo(() => {
    ...
    entries.forEach((entry) => {
      const date = dateValue(entry.values[dateProperty.id]);
      if (!date) return;
  ...
  if (loading)
    return (
      <div className="flex items-center gap-2 p-6 text-sm text-muted-foreground">
        <Loader2 className="h-4 w-4 animate-spin" /> Kalender laden…
      </div>
    );
  if (!dateProperty)
    return (
      <div className="rounded-lg border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
        Voeg een date-property toe om een kalender te maken.
      </div>
    );
    ...
    <section className="rounded-lg border border-border bg-card">
      ...
      <div className="grid grid-cols-7 border-b border-border">
        {['ma', 'di', 'wo', 'do', 'vr', 'za', 'zo'].map((day) => (
          <div
            key={day}
            className="px-2 py-2 text-center text-[11px] uppercase tracking-[0.12em] text-muted-foreground"
          >
            {day}
          </div>
        ))}
        ...
        <>
          {days.map((day) => {
            ...
            return (
              <div
                key={day.toISOString()}
                className={`min-h-28 border-b border-r border-border p-2 ${!isSameMonth(day, month) ? 'bg-secondary/20 text-muted-foreground' : ''}`}
              >
                <span
                  className={`grid h-6 w-6 place-items-center rounded-full text-xs ${isSameDay(day, new Date()) ? 'bg-primary text-primary-foreground' : ''}`}
                >
                  {format(day, 'd')}
                </span>
                <div className="mt-1 space-y-1">
                  {dayEntries.map((entry) => (
                    <div
                      key={entry.id}
                      className="truncate rounded bg-secondary px-1.5 py-1 text-[11px]"
                      title={textValue(
                        titleProperty
