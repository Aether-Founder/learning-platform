...
export function StudyPlanWizard({ userId, onComplete, onCancel }: StudyPlanWizardProps) {
  ...
  const renderStep1 = () => (
    <div className="space-y-6">
      ...
      <div>
        ...
        <Select value={planData.goalType} onValueChange={(value: any) => setPlanData({ ...planData, goalType: value })}>
          <SelectTrigger>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
    ...
    <div className="space-y-6">
      <div>
        ...
        <Input
          ...
          onChange={(e) => setPlanData({ ...planData, dailyGoalMinutes: parseInt(e.target.value) || 0 })}
          min="15"
          max="480"
        />
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2 mt-2">
          {["Ochtend (06:00-12:00)", "Middag (12:00-18:00)", "Avond (18:00-22:00)", "Nacht (22:00-06:00)"].map((time) => (
    ...
    <div className="space-y-6">
      <div className="bg-muted/50 p-4 rounded-lg">
        ...
        <div className="space-y-2 text-sm">
          <div><span className="font-medium">Titel:</span> {planData.title}</div>
          <div><span className="font-medium">Type:</span> {planData.goalType}</div>
          {planData.goalType === "exam" && (
            <div><span className="font-medium">Doeldatum:</span> {planData.targetDate.toLocaleDateString('nl-NL')}</div>
          )}
          <div><span className="font-medium">Dagelijks Doel:</span> {planData.dailyGoalMinutes} minuten</div>
          <div><span className="font-medium">Voorkeurstijden:</span> {planData.preferredTimes.join(", ") || "Geen"}</div>
          <div><span className="font-medium">Studie Sets:</span> {planData.selectedStudySets.length} geselecteerd</div>
        </div>
      </div>
    </div>
  );
