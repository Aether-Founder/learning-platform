    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {mode === 'register' && (
            <div>
              <label htmlFor="modal-confirm-password" className="mb-2 block text-sm font-medium">Bevestig wachtwoord</label>
              <input id="modal-confirm-password" type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" placeholder="Herhaal je wachtwoord" autoComplete="new-password" minLength={6} required />
            </div>
          )}
          ...
          <button type="button" onClick={async () => {
            ...
            try {
              ...
              setError(oauthError instanceof Error ? oauthError.message : 'Er is een fout opgetreden bij Google login.');
            } finally {
              setLoading(false);
            }
          }} disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-md border border-border bg-background px-4 py-2.5 text-sm font-medium hover:bg-accent disabled:opacity-50">
