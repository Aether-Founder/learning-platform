  ...
  const practiceRows = useMemo(() => {
    ...
    return subs.map(sub => {
      ...
      const subjectSets = sets.filter(s => {
        ...
        return s.title.toLowerCase().includes(subName) || 
      ...
      if (subjectSets.length > 0) {
        ...
        const studiedCards = subjectSets.reduce((sum, s) => sum + s.cards.filter(c => c.attempts > 0).length, 0);
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-background/90 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-[1400px] items-center gap-8 px-8">
          ...
          <button type="button" onClick={() => setScreen("home")} className="flex items-center gap-3">
            ...
            <span style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl font-normal tracking-tight">Aether</span>
          </button>
        ...
        <div className="mx-auto max-w-[1400px] space-y-12 px-8 pb-24">
          ...
          <section className="grid gap-8 border-b border-border py-12 md:grid-cols-[1.4fr_1fr] md:items-end">
            <div>
              ...
              <h1 style={{ fontFamily: 'Cormorant Garamond, serif' }} className="mt-3 text-5xl font-semibold leading-[1.05]">
            ...
            <dl className="grid grid-cols-3 gap-6 md:border-l md:border-border md:pl-8">
              <div className="flex flex-col gap-0.5">
                <span className="font-display text-3xl font-semibold leading-none">{sets.length}</span>
                <span className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Sets</span>
              </div>
              <div className="flex flex-col gap-0.5">
                <span className="font-display text-3xl font-semibold leading-none">{userProfile.selectedSubjects.length}</span>
                <span className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Vakken</span>
              </div>
              <div className="flex flex-col gap-0.5">
                <span className="font-display text-3xl font-semibold leading-none">{sets.reduce((sum, s) => sum + s.cards.filter(c => c.attempts === 0).length, 0)}</span>
                <span className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Te herhalen</span>
              </div>
            </dl>
          </section>
            ...
            <aside className="space-y-10 lg:sticky lg:top-24 lg:self-start">
              ...
              <div>
                ...
                <div className="flex items-end gap-1.5">
                  {(() => {
                    ...
                    const activityByDay = last7Days.map(dateStr => {
                      const cardsStudied = sets.reduce((sum, set) => 
                        sum + set.cards.filter(c => 
                ...
                <p className="mt-4 text-xs leading-relaxed text-muted-foreground">
                  {(() => {
                    const totalStudied = sets.reduce((sum, s) => sum + s.cards.filter(c => c.attempts > 0).length, 0);
