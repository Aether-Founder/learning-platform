    ...
    <AppShell>
      <PageHeader
        ...
        action={
          ...
          >
            {t('analytics_fsrs_link')} <ArrowUpRight className="h-4 w-4" />
          </Link>
        }
      />

      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {metrics.map(([value, labelKey, Icon]) => {
          const MetricIcon = Icon as typeof Clock3;
          return (
            <div key={labelKey} className="rounded-xl border border-border bg-card p-5">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_1fr]">
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              <h2 className="mt-2 font-display text-2xl font-semibold">
                {t('analytics_week_time')}
              </h2>
            </div>
            <span className="text-xs text-muted-foreground">{t('analytics_week_note')}</span>
          </div>
          <div className="mt-8 flex h-48 items-end gap-3">
            {DAYS.map((item) => (
              <div key={item.day} className="flex flex-1 flex-col items-center gap-2">
                <div className="relative flex h-40 w-full items-end rounded-sm bg-secondary">
                  <div
                    className="w-full rounded-sm bg-foreground/70 transition-all"
                    style={{ height: `${item.value}%` }}
                  />
                </div>
                <span className="text-[11px] text-muted-foreground">{t(DAY_KEYS[item.day])}</span>
              </div>
            ))}
          </div>
        </section>
        <section className="rounded-xl border border-border bg-card p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[11px] uppercase tracking-[0.16em] text-muted-foreground">
                {t('analytics_mastery_section')}
              </p>
              <h2 className="mt-2 font-display text-2xl font-semibold">
                {t('analytics_per_subject')}
              </h2>
            </div>
            <span className="text-xs text-muted-foreground">{t('analytics_subject_note')}</span>
          </div>
            ...
            {SUBJECTS.map((subject) => (
              <div key={subject.name}>
                <div className="mb-2 flex justify-between text-sm">
                  ...
                  <span className="tabular-nums text-muted-foreground">{subject.value}%</span>
                </div>
      ...
      <div className="mt-6 flex items-center gap-2 text-xs text-muted-foreground">
        <Trophy className="h-4 w-4" /> {t('analytics_footer')}
      </div>
    </AppShell>
  );
}
