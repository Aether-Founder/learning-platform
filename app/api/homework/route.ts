...
export async function GET(request: NextRequest) {
  try {
    ...
    let homework;
    if (status === "pending") {
      homework = await getHomeworkByUserId(payload.userId).then(hw => hw.filter(h => h.status !== "completed"));
    } else if (subject) {
      homework = await getHomeworkByUserId(payload.userId).then(hw => hw.filter(h => h.subject === subject));
    } else {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, description, subject, dueDate, priority, estimatedTime, testWeekId, relatedStudySetId } = body;
