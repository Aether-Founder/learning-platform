...
export async function GET(request: NextRequest) {
  try {
    ...
    let studyPlans;
    if (testWeekId) {
      studyPlans = await getStudyPlansByUser(payload.userId).then(sp => sp.filter(sp => sp.testWeekId === testWeekId));
    } else {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!testWeekId || !name || !startDate || !endDate) {
      return NextResponse.json({ error: "Test week ID, name, start date, and end date are required" }, { status: 400 });
    }
