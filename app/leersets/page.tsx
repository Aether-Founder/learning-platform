...
export default function LeersetsPage() {
  ...
  const handleDeleteSet = async (setId: string) => {
    if (!confirm('Weet je zeker dat je deze leerlijst wilt verwijderen? Alle kaarten worden ook verwijderd.')) return;
  ...
  const openEditSetDialog = (set: StudySet) => {
    router.push(`/edit/leerlijst/${set.id}`);
  };
  ...
  if (loading) {
    return (
      <AppShell>
        <PageHeader
          eyebrow={t('leersets_eyebrow')}
          title={t('leersets_title')}
          description={t('leersets_description')}
        />
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredSets.length === 0 ? (
            <div className="col-span-full text-center py-12 border-2 border-dashed border-border rounded-lg">
              ...
              <p className="text-lg font-medium mb-2">{t('leersets_empty')}</p>
              <p className="text-sm text-muted-foreground mb-4">{t('leersets_empty_desc')}</p>
              <Button onClick={openCreateSetDialog}>
                <Plus className="mr-2 h-4 w-4" />
                {t('leersets_empty_cta')}
              </Button>
            </div>
          ) : (
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="Wat leer je in deze leerlijst?"
                rows={3}
              />
            </div>
          </div>
