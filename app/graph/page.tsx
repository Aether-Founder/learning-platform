...
const NODES = [
  ...
  },
  {
    ...
    x: '79%',
    y: '22%',
    detail: '18 kaarten · 41% beheerst',
  },
  {
    id: 'kracht',
    label: 'Kracht & beweging',
    kind: 'concept',
    x: '20%',
    y: '72%',
    detail: 'Natuurkunde · hoofdstuk 3',
  },
  {
    id: 'formules',
    label: 'Formules',
      ...
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-2 rounded-full bg-secondary px-3 py-1.5 text-xs text-muted-foreground">
            <Network className="h-3.5 w-3.5" /> {t('graph_badge')}
          </div>
        }
      />
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_280px]">
          ...
          <div className="relative h-[510px]">
            ...
            {NODES.map((node) => (
              ...
              >
                ...
                <span
                  className={`mt-1 block text-[10px] uppercase tracking-[0.12em] ${selected === node.id ? 'text-background/70' : 'text-muted-foreground'}`}
                >
                  {t(KIND_KEYS[node.kind])}
                </span>
              </button>
            ))}
          </div>
        ...
        <aside className="rounded-xl border border-border bg-card p-5">
          <div className="relative">
            ...
            />
            <input
              aria-label={t('graph_search')}
              placeholder={t('graph_search')}
              className="h-10 w-full rounded-md border border-border bg-background pl-9 pr-3 text-sm outline-none focus:border-foreground/40"
            />
          </div>
          <div className="mt-8">
          ...
          <div className="mt-7 space-y-3 border-t border-border pt-5">
            <div className="flex items-center gap-3 text-sm">
              <BookOpen className="h-4 w-4 text-muted-foreground" /> {t('graph_linked_lessons')}
            </div>
            <div className="flex items-center gap-3 text-sm">
              <Link2 className="h-4 w-4 text-muted-foreground" /> {t('graph_related_concepts')}
            </div>
          </div>
          <button
            type="button"
            className="mt-8 h-10 w-full rounded-md bg-primary text-sm font-medium text-primary-foreground hover:opacity-90"
          >
            {t('graph_start_review')}
          </button>
        </aside>
      </div>
    </AppShell>
