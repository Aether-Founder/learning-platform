...
export default function StudyModePage() {
  ...
  const [testResults, setTestResults] = useState<
    Record<string, { correct: boolean; similarity: number }>
  >({});
  ...
  const sessionProgress = cards.length
    ? Math.round(((index + (complete ? 1 : 0)) / cards.length) * 100)
    : 0;
  ...
  if (error)
    return (
      <AppShell>
        <div className="mx-auto mt-16 max-w-md rounded-xl border border-rose-500/30 bg-rose-500/5 p-6 text-center text-sm text-rose-500">
          {error}
        </div>
      </AppShell>
    );
  if (!cards.length)
    return (
      <AppShell>
        <div className="mx-auto mt-16 max-w-md rounded-xl border border-dashed border-border p-8 text-center">
          <p className="font-display text-2xl font-semibold">{t('study_no_cards_title')}</p>
          <p className="mt-2 text-sm text-muted-foreground">{t('study_no_cards_desc')}</p>
        </div>
      </AppShell>
    );
    ...
    <AppShell>
      <div className="mx-auto max-w-3xl py-10">
        ...
        <div className="mt-8 flex items-center justify-between text-xs text-muted-foreground">
          <span>
            {t('study_progress', undefined, {
              n: Math.min(index + 1, cards.length),
              total: cards.length,
            })}
          </span>
        ...
        {complete ? (
          <div className="mt-14 rounded-xl border border-border bg-card p-6 sm:p-10 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              {t('study_complete_text', undefined, { n: cards.length })}
            </p>
              ...
              <div className="mt-6 flex items-center justify-center gap-4 sm:gap-6 flex-wrap">
                ...
                <div className="flex items-center gap-2 text-orange-500">
                  ...
                  <span className="font-semibold">
                    {gamificationState.currentStreak} dag streak
                  </span>
                </div>
              </div>
            )}
          ...
          <>
            {mode === 'flashcards' && (
              <>
                ...
                >
                  ...
                  <span className="mt-10 block text-xs text-muted-foreground">
                    {t('study_flip_hint')}
                  </span>
                </button>
            ...
            {mode === 'learn' && (
              <div className="mt-10 rounded-2xl border border-border bg-card p-10 text-center shadow-sm">
                ...
                {showFeedback && (
                  <div
                    className={`mt-6 p-4 rounded-lg ${isCorrect ? 'bg-emerald-500/10 text-emerald-700' : 'bg-rose-500/10 text-rose-700'}`}
                  >
                    {isCorrect
                      ? 'Correct!'
                      : `Niet correct. Het juiste antwoord is: ${card.answer}`}
                  </div>
                )}
                <button
                  onClick={handleNext}
            ...
            {mode === 'write' && (
              <div className="mt-10 rounded-2xl border border-border bg-card p-10 text-center shadow-sm">
                ...
                {writeFeedback && (
                  <div
                    className={`mt-4 p-4 rounded-lg ${writeCorrect ? 'bg-emerald-500/10 text-emerald-700' : 'bg-rose-500/10 text-rose-700'}`}
                  >
                    {writeFeedback}
                  </div>
                )}
                  ...
                  <>
                    <div className="rounded-2xl border border-border bg-card p-10 text-center shadow-sm">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setTestAnswers({ ...testAnswers, [card.id]: e.target.value })
                        }
                        className="mt-8 w-full max-w-md mx-auto p-4 rounded-lg border border-border bg-background min-h-[100px]"
                        placeholder="Typ je antwoord hier..."
                      />
                    ...
                    <div className="mt-6 space-y-4">
                      {cards.map((c, i) => {
                        ...
                        return (
                          <div
                            key={c.id}
                            className={`p-4 rounded-lg border ${result?.correct ? 'border-emerald-500 bg-emerald-500/5' : 'border-rose-500 bg-rose-500/5'}`}
                          >
                            <p className="font-medium">
                              {i + 1}. {c.question}
                            </p>
                            <p className="mt-2 text-sm text-muted-foreground">
                              Jouw antwoord: {answer || '(geen antwoord)'}
                            </p>
                            <p className="mt-1 text-sm text-muted-foreground">
                              Juiste antwoord: {c.answer}
                            </p>
                            <p className="mt-1 text-xs">
                              Overeenkomst:{' '}
                              {result?.similarity ? Math.round(result.similarity * 100) : 0}%
                            </p>
                          </div>
                        );
                      })}
                ...
                {matchedPairs.length > 0 && (
                  <div className="mt-6 p-4 rounded-lg bg-emerald-500/10 border border-emerald-500">
                    <p className="text-sm text-emerald-700">
                      Gekoppelde paren: {matchedPairs.length}
                    </p>
                  </div>
                )}
                <button
