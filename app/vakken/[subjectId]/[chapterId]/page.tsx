import { loadSubject, loadChapters, loadLearningSet, loadQuiz, loadSummary, loadPracticeTest, type Chapter } from '@/lib/content-loader';
    ...
    return (
      <AppShell fullWidth>
        <div className="text-center py-20">
          ...
          <Link href={`/vakken/${subjectId}`} className="mt-4 inline-block text-primary hover:underline">
            Terug naar vak
          </Link>
        </div>
      </AppShell>
        ...
        <div className="border border-border rounded-lg overflow-hidden">
          <div className="bg-muted/50 px-4 py-2 border-b border-border">
            <span className="text-sm text-muted-foreground">
              {chapter.content.learningSets.length + chapter.content.quizzes.length + chapter.content.summaries.length + chapter.content.practiceTests.length} items
            </span>
          </div>
                ...
                {chapter.content.learningSets.map((lsId) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-medium truncate">{lsId.replace(/-/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}</div>
              ...
              <div>
                ...
                {chapter.content.quizzes.map((quizId) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-medium truncate">{quizId.replace(/-/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}</div>
              ...
              <div>
                ...
                {chapter.content.summaries.map((summaryId) => (
                  ...
                  >
                    <FileText className="h-5 w-5 text-primary" />
                    <div className="flex-1 min-w-0">
                      <div className="font-medium truncate">{summaryId.replace(/-/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}</div>
              ...
              <div>
                ...
                {chapter.content.practiceTests.map((testId) => (
                  ...
                  >
                    <FileText className="h-5 w-5 text-primary" />
                    <div className="flex-1 min-w-0">
                      <div className="font-medium truncate">{testId.replace(/-/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}</div>
