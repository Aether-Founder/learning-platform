      ...
      <div className="mx-auto mt-10 max-w-3xl">
        <section className="overflow-hidden rounded-xl border border-border bg-card">
          <div className="flex min-h-[330px] flex-col items-center justify-center p-8 text-center">
            <div
              className={`grid h-28 w-28 place-items-center rounded-full border ${recording ? 'border-rose-500/50 bg-rose-500/10' : 'border-border bg-secondary'}`}
            >
              <Mic
                className={`h-10 w-10 ${recording ? 'text-rose-500' : 'text-muted-foreground'}`}
              />
            </div>
            ...
            <div className="mt-8 flex items-center gap-3">
              {!recording ? (
                ...
                >
                  <Play className="h-4 w-4" /> {t('record_start')}
                </button>
              ) : (
                <>
                  <button
                    ...
                    aria-label={paused ? t('record_resume') : t('record_pause')}
                  >
                  ...
                  <button
                    type="button"
                    onClick={stop}
                    className="inline-flex h-11 items-center gap-2 rounded-md bg-rose-600 px-5 text-sm font-medium text-white hover:bg-rose-700"
                  >
                    <Square className="h-4 w-4" /> {t('record_stop')}
                  </button>
                </>
              )}
            </div>
          ...
          {audioUrl && (
            <div className="flex flex-wrap items-center gap-4 border-t border-border p-5">
              ...
              <audio className="min-w-[220px] flex-1" controls src={audioUrl}>
                {t('record_unsupported')}
              </audio>
              <a
                href={audioUrl}
                download="aether-college.webm"
              ...
              >
                <Download className="h-4 w-4" /> {t('record_download')}
              </a>
              <button
                type="button"
                onClick={discard}
