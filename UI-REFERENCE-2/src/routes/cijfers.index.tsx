...
export const Route = createFileRoute("/cijfers/")({
  head: () => ({
    meta: [
      { title: "Cijfers — Aether" },
      {
        name: "description",
        content:
          "Cijferoverzicht per vak en per toets: gemiddelden, weging, periodes en de rekenhulp voor je streefcijfer.",
      },
      { property: "og:title", content: "Cijfers — Aether" },
      {
        property: "og:description",
        content: "Alle cijfers per vak en per toets, met gemiddelden en streefcijferberekening.",
      },
    ],
  }),
  component: CijfersPage,
});
      ...
      <div className="pt-8">
        ...
        {view === "periodes" && (
          <div className="grid gap-6 md:grid-cols-2">
            {GRADEBOOK.map((s) => (
              <Panel key={s.slug} title={s.name} action={<Badge tone={gradeTone(averageOf(s.grades))}>{fmt(averageOf(s.grades))}</Badge>}>
                <KeyValue
                  items={([1, 2, 3, 4] as const).map((p) => {
                    const list = s.grades.filter((g) => g.period === p);
                    return [
                      `Periode ${p}`,
                      list.length ? (
                        <span className="tabular-nums">{fmt(averageOf(list))}</span>
                      ) : (
