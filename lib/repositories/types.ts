...
export interface IUserRepository {
  ...
  updateProfile(id: string, updates: Partial<Pick<User, 'displayName' | 'avatar' | 'school' | 'grade'>>): Promise<User | null>;
}
...
export interface IStudySetRepository {
  ...
  create(userId: string, data: { title: string; description?: string; folderId?: string; isPublic?: boolean }): Promise<StudySet>;
  update(id: string, updates: Partial<Pick<StudySet, 'title' | 'description' | 'folderId' | 'isPublic'>>): Promise<StudySet | null>;
...
export interface ICardRepository {
  ...
  addCard(studySetId: string, data: Omit<StudyCard, 'id' | 'studySetId' | 'createdAt'>): Promise<StudyCard>;
  updateCard(cardId: string, updates: Partial<Omit<StudyCard, 'id' | 'studySetId' | 'createdAt'>>): Promise<StudyCard | null>;
...
export interface ICalendarRepository {
  ...
  createEvent(userId: string, data: Omit<CalendarEvent, 'id' | 'userId' | 'createdAt' | 'updatedAt'>): Promise<CalendarEvent>;
...
export interface ITestWeekRepository {
  ...
  createTestWeek(userId: string, data: { name: string; startDate: string; endDate: string }): Promise<TestWeek>;
...
export interface IHomeworkRepository {
  ...
  createHomework(userId: string, item: Omit<Homework, 'id' | 'userId' | 'createdAt' | 'status'>): Promise<Homework>;
